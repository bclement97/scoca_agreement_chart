...
It is recommended to always use OPINION_REGEX with re.findall() or
re.finditer() since it will always match OPINION_REGEX_MAJORITY before
matching OPINION_REGEX_SECONDARY. If not, matches of
OPINION_REGEX_MAJORITY must be removed from the search string before
attempting to match OPINION_REGEX_SECONDARY which can wrongly include
what should be matched by OPINION_REGEX_MAJORITY.

The unicode and case-insensitive flag should be set in most cases. This
is because this regex may not always match due to typos, or minor
differences in punctuation between published opinions and may need to be
...
- OPINION: groups of OPINION_MAJORITY followed by groups of
OPINION_SECONDARY
"""
...
OPINION_MAJORITY = (_OPINION_AUTHOR + 'the opinion of the court'
                    + _OPINION_CONCURRING)
# Like OPINION_REGEX_MAJORITY, except for all other opinions.
OPINION_SECONDARY = (
    _OPINION_AUTHOR
    + 'a (concurring|dissenting|concurring and dissenting) opinion(?:'
    + _OPINION_CONCURRING + ')?'
)
...
def normalize_whitespace(text):
    """Converts literal newlines ('\' followed by 'n') and whitespace
    (including Unicode) to a single space character.
    """
